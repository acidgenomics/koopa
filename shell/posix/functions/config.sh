...
koopa::enable_passwordless_sudo() { # {{{1
    ...
    koopa::h2 "Updating '${sudo_file}' to include '${group}'."
    local string
...
koopa::enable_shell() { # {{{1
    ...
    etc_file="/etc/shells"
    [ -f "$etc_file" ] || return 0
    koopa::h2 "Updating '${etc_file}' to include '${cmd_path}'."
...
koopa::link_docker() { # {{{1
    ...
    koopa::h2 "Updating Docker configuration."
    koopa::assert_is_linux
...
koopa::update_xdg_config() { # {{{1
    ...
    if [ ! -e "${HOME:-}" ]
    then
        koopa::warning "HOME does not exist: '${HOME:-}'."
        return 0
    fi
    koopa_prefix="$(koopa::prefix)"
    config_prefix="$(koopa::config_prefix)"
