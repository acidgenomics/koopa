...
#+STARTUP: content
* High priority
** TODO 'kdev' isn't working inside of koopa Debian docker build.
    Actually this is working correctly, it's just not setting the prompt as expected.
** TODO Add install support for Nanopore guppy caller.
...
** TODO Consider adding install support for arvados (linux).
