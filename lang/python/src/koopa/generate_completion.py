...
def _generate_fish_completion(
    ...
    for cmd in _TOP_CMDS:
        lines.append(
            f"complete -c koopa"
            f" -n 'not __fish_seen_subcommand_from {top_seen}'"
            f" -a '{cmd}'"
        )
    ...
    for install_cmd in ("install", "reinstall", "uninstall"):
        for app in all_apps:
            lines.append(
                f"complete -c koopa"
                f" -n '__fish_seen_subcommand_from {install_cmd}'"
                f" -a '{app}'"
            )
...
def generate_completion() -> None:  # noqa: PLR0915
    ...
    bash_path = os.path.join(
        koopa_prefix(), "share", "bash-completion", "completions", "koopa"
    )
    ...
    fish_path = os.path.join(
        koopa_prefix(), "share", "fish", "vendor_completions.d", "koopa.fish"
    )
    ...
    zsh_path = os.path.join(
        koopa_prefix(), "share", "zsh", "site-functions", "_koopa"
    )
