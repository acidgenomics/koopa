...
_ADMIN_COMMANDS: list[tuple[str, str | None]] = [
    ...
    ("install-app", "linux"),
    ("os-version", "linux"),
    ("pacman-install", "linux"),
    ("pacman-remove", "linux"),
    ("pacman-update", "linux"),
    ("proc-cmdline", "linux"),
    ("systemctl-disable", "linux"),
    ("systemctl-restart", "linux"),
    ("systemctl-status", "linux"),
    ("systemctl-stop", "linux"),
    ("uninstall-app", "linux"),
    ("zypper-install", "linux"),
    ("zypper-remove", "linux"),
    ("zypper-update", "linux"),
...
# ---------------------------------------------------------------------------
# Elvish completion generator
# ---------------------------------------------------------------------------


def _generate_elvish_completion(
    app_tree: dict[str, Any],
    develop_cmds: list[str],
    common_apps: list[str],
    linux_apps: list[str],
    ...
    lines.append("        var path = ''")
    lines.append("        if (>= $n 3) {")
    lines.append("            set path = $args[1]")
    lines.append("        }")
    lines.append("        if (>= $n 4) {")
    lines.append("            set path = $path'/'$args[2]")
    lines.append("        }")
    lines.append("        if (>= $n 5) {")
    lines.append("            set path = $path'/'$args[3]")
    lines.append("        }")

    # Emit flag map as elvish conditionals.
    first = True
    for path_key in sorted(flag_map):
        flags = sorted(flag_map[path_key])
        keyword = "if" if first else "} elif"
        lines.append(f"        {keyword} (eq $path '{path_key}') {{")
        lines.append(f"            put {' '.join(flags)}")
        first = False
    if not first:
        lines.append("        }")
    lines += [
        ...
        f"        put --help --version {' '.join(_TOP_CMDS)}",
        "    } elif (== $n 3) {",
        "        # Depth 1: subcommands of top-level.",
    ]

    # Per-top-level-command dispatch at depth 1.
    lines.append("        if (eq $args[1] 'app') {")
    lines.append(f"            put {' '.join(app_ns)}")
    lines.append("        } elif (eq $args[1] 'develop') {")
    lines.append(f"            put {' '.join(develop_cmds)}")
    lines.append("        } elif (eq $args[1] 'run') {")
    lines.append(f"            put {' '.join(run_cmds)}")
    lines.append("        } elif (eq $args[1] 'system') {")
    lines.append(f"            put {' '.join(system_cmds)}")
    lines.append("        } elif (eq $args[1] 'admin') {")
    lines.append(f"            put {' '.join(admin_cmds)}")
    lines.append("        } elif (eq $args[1] 'install') {")
    lines.append(f"            put {' '.join(all_apps)}")
    lines.append("        } elif (eq $args[1] 'reinstall') {")
    lines.append(f"            put {' '.join(all_apps)}")
    ...
    for ns in app_ns:
        ...
        if isinstance(val, dict):
            children = sorted(val.keys())
            keyword = "if" if first_ns else "} elif"
            lines.append(f"            {keyword} (eq $args[2] '{ns}') {{")
            lines.append(f"                put {' '.join(children)}")
            first_ns = False
    if not first_ns:
        lines.append("            }")
    lines.append("        }")
    ...
    depth4 = _collect_app_depth_4(app_tree)
    first_d4 = True
...
def _generate_nushell_completion(
    app_tree: dict[str, Any],
    develop_cmds: list[str],
    common_apps: list[str],
    linux_apps: list[str],
    ...
    # Completer functions.
    _nu_completer(lines, "koopa_app_names", all_apps)
    _nu_completer(lines, "koopa_app_namespaces", app_ns)
    _nu_completer(lines, "koopa_develop_cmds", develop_cmds)
    ...
    for ns in app_ns:
        ...
        if isinstance(val, dict):
            children = sorted(val.keys())
            fn_name = f"koopa_app_{ns.replace('-', '_')}_cmds"
            _nu_completer(lines, fn_name, children)
    ...
    # install/reinstall/uninstall externs.
    for cmd in ("install", "reinstall", "uninstall"):
        cmd_flags = sorted(flag_map.get(cmd, []))
        lines.append(f'extern "koopa {cmd}" [')
        for f in cmd_flags:
            lines.append(f"    {f}")
        lines.append("    app_name: string@koopa_app_names")
        lines.append("]")
        lines.append("")

    # update extern.
    update_flags = sorted(flag_map.get("update", []))
    lines.append('extern "koopa update" [')
    ...
    # configure extern.
    lines += [
        'extern "koopa configure" [',
        "    mode: string@koopa_configure_modes",
        "]",
        "",
    ]

    # develop extern.
    dev_flags_all: list[str] = []
    for key, flags in sorted(flag_map.items()):
        if key.startswith("develop/"):
            dev_flags_all.extend(flags)
    lines.append('extern "koopa develop" [')
    lines.append("    cmd: string@koopa_develop_cmds")
    lines.append("]")
    lines.append("")

    # Per-develop-command externs with flags.
    for cmd in develop_cmds:
        flags = flag_map.get(f"develop/{cmd}", [])
        if flags:
            lines.append(f'extern "koopa develop {cmd}" [')
    ...
    lines += [
        'extern "koopa admin" [',
        "    cmd: string@koopa_admin_cmds",
        "]",
        "",
    ]

    # app extern (top-level namespace).
    lines += [
        'extern "koopa app" [',
        "    namespace: string@koopa_app_namespaces",
        "]",
        "",
    ]

    # Per-namespace app externs.
    for ns in app_ns:
        val = app_tree[ns]
        if isinstance(val, dict):
            fn_name = f"koopa_app_{ns.replace('-', '_')}_cmds"
            lines.append(f'extern "koopa app {ns}" [')
            lines.append(f"    cmd: string@{fn_name}")
            lines.append("]")
            lines.append("")
            # Sub-namespace externs.
            for sub, sub_val in sorted(val.items()):
                if isinstance(sub_val, dict):
                    sub_fn = f"koopa_app_{ns.replace('-', '_')}_{str(sub).replace('-', '_')}_cmds"
                    lines.append(f'extern "koopa app {ns} {sub}" [')
                    lines.append(f"    cmd: string@{sub_fn}")
                    lines.append("]")
                    lines.append("")
...
def generate_completion() -> None:  # noqa: PLR0915
    ...
    with open(elvish_path, "w") as f:
        f.write(elvish_content)
    ...
    with open(nushell_path, "w") as f:
        f.write(nushell_content)
