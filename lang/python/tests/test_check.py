...
def test_check_dracula_pro_update_available_skips_when_not_installed() -> None:
    """Does nothing, and never calls check(), when Dracula Pro isn't installed."""
    from koopa.check import check_dracula_pro_update_available

    with (
        patch("koopa.dracula_pro.installed_version", return_value=None),
        patch("koopa.dracula_pro.check") as mock_check,
    ):
        check_dracula_pro_update_available()
        mock_check.assert_not_called()


def test_check_dracula_pro_update_available_calls_check_when_installed() -> None:
    """Delegates to dracula_pro.check() when a version is on record."""
    from koopa.check import check_dracula_pro_update_available

    with (
        patch("koopa.dracula_pro.installed_version", return_value="2.2.3"),
        patch("koopa.dracula_pro.check") as mock_check,
    ):
        check_dracula_pro_update_available()
...
def test_check_dracula_pro_update_available_swallows_network_error() -> None:
    """A network failure while checking the changelog feed doesn't raise."""
    import urllib.error
    ...
    with (
        patch("koopa.dracula_pro.installed_version", return_value="2.2.3"),
        patch("koopa.dracula_pro.check", side_effect=urllib.error.URLError("offline")),
    ):
        check_dracula_pro_update_available()  # must not raise


def test_check_dracula_pro_update_available_swallows_value_error() -> None:
    """A changelog feed with no version mention doesn't raise."""
    from koopa.check import check_dracula_pro_update_available

    with (
        patch("koopa.dracula_pro.installed_version", return_value="2.2.3"),
        patch("koopa.dracula_pro.check", side_effect=ValueError("no version found")),
    ):
        check_dracula_pro_update_available()  # must not raise
