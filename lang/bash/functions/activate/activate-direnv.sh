...
_koopa_activate_direnv() {
    ...
    # re-runs on every 'cd' via the 'PROMPT_COMMAND' hook that 'direnv hook'
    # installs, not just at shell startup. Both call sites are bound with
    # 'gtimeout' so a pathological '.envrc' can't wedge the shell on startup
    # OR on a later 'cd'. '_direnv_hook' is redefined (after sourcing the
    # cached hook) to reference 'KOOPA_PREFIX' / 'KOOPA_DIRENV_TIMEOUT'
    # directly rather than this function's locals, since it fires from
    # 'PROMPT_COMMAND' long after this function has returned and its locals
