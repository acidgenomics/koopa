...
install_from_conda() {
    _koopa_install_conda_package --name='sra-tools'
    return 0
}
...
${dict['version']}.tar.gz"
    _koopa_download "${dict['vdb_url']}" 'ncbi-vdb.tar.gz'
    _koopa_extract 'ncbi-vdb.tar.gz' 'ncbi-vdb'
    dict['sra_url']="https://github.com/ncbi/sra-tools/archive/refs/tags/\
${dict['version']}.tar.gz"
    _koopa_download "${dict['sra_url']}" 'sra-tools.tar.gz'
    _koopa_extract 'sra-tools.tar.gz' 'sra-tools'
    if _koopa_is_root
    then
    ...
    _koopa_append_cflags '-DH5_USE_110_API'
    ...
    cmake_args=(
        ...
        "-DPython3_EXECUTABLE=${cmake['python3_executable']}"
        "-DVDB_LIBDIR=$(_koopa_realpath 'ncbi-vdb/lib')"
    )
    "${app['cmake']}" \
        -B 'sra-tools' \
        -S "$(_koopa_realpath '../sra-tools')" \
        "${cmake_args[@]}"
    "${app['cmake']}" \
        --build 'sra-tools' \
        --parallel "${dict['jobs']}" \
