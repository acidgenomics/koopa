...
main() {
    ...
    # """
    local -A dict
    _koopa_activate_app --build-only 'curl'
    dict['build_prefix']="$(_koopa_init_dir 'build')"
...
tags/v${dict['version']}.tar.gz"
    ...
    _koopa_assert_is_file "${dict['bootstrap']}"
    "${dict['bootstrap']}"
    _koopa_cp "${dict['build_prefix']}/.ghcup" "${dict['prefix']}/libexec"
    (
