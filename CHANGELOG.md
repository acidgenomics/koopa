...
- Added `koopa app r publish-docs`, converging `r.acidgenomics.com`'s pkgdown
  docs layout onto the same `/<name>/` convention as
  `python.acidgenomics.com`. Docs moved from `/packages/<name>/` to `/<name>/`
  (R has no artifact collision there, unlike Python's `/packages/` which
  holds wheels/sdists and must keep that layout). An S3 website `RoutingRule`
  301-redirects the old `/packages/<name>/...` paths. Fixes a live bug where
  `AcidDevTools::pkgdownDeployToAws()` defaulted to
  `s3://r.acidgenomics.com/packages/`, a legacy bucket holding zero objects —
  every pkgdown deploy through it was silently writing into a void.
  `AcidDevTools::publish()` and `pkgdownDeployToAws()` now delegate to the new
  koopa command. Implementation: `publish_docs()` in `cran.py`, mirroring
  `pypi.py`'s function of the same name.
- `r.acidgenomics.com`'s landing page is now generated by `reindex`/`publish`
  instead of hand-edited and re-uploaded. Extracted the shared HTML skeleton
  (head, nav, breadcrumb, search form, footer) into a new `render_landing()` in
  `landing.py`, used by both `pypi.py` and `cran.py`; `pypi.py`'s generated
  output is unchanged (verified byte-identical against the live page). The R
  page adds a curated `_CATEGORIES` map so package listings keep their
  existing section groupings; any package present in `src/contrib` but absent
  from every category now surfaces under a new "Infrastructure" section with a
  warning, instead of silently vanishing (8 previously-hidden packages: `AcidBase`,
  `AcidCLI`, `AcidDevTools`, `AcidGenerics`, `AcidMarkdown`, `AcidRoxygen`,
  `AcidTest`, `basejump`). Also fixes a stale footer that claimed AGPLv3; every
  R package has been Apache 2.0 since the relicense.
