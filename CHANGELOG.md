...
- Moved the private Python package index from the bare domain root to
  `/simple/` (PEP 503 convention) and consolidated Python and R docs onto
  the same domain convention: `publish-docs` now syncs to
  `python.acidgenomics.com/<name>/` and the new `koopa app r publish-docs`
  syncs to `r.acidgenomics.com/<name>/` (moved from `/packages/<name>/`,
  with a 301 `RoutingRule` for old paths). The R move fixes a live bug
  where `AcidDevTools::pkgdownDeployToAws()` defaulted to a legacy bucket
  holding zero objects, silently writing every pkgdown deploy into a void;
  `AcidDevTools::publish()` now delegates to the new koopa command.
- Both `python.acidgenomics.com` and `r.acidgenomics.com` landing pages are
  now generated by `reindex`/`publish` instead of hand-edited, via a shared
  `render_landing()` in `landing.py`. The R page's category groupings
  surface any uncategorized package under a new "Infrastructure" section
  with a warning instead of silently vanishing (8 previously-hidden
  packages), and fixes a stale footer that claimed AGPLv3 instead of
  Apache 2.0. A shared acidgenomics CSS theme (`css/sphinx.css`) matches
  Sphinx docs to the site's existing pkgdown look; `koopa app python
  publish-assets` publishes it.
- Non-interactive shells (`ssh host cmd`, CI steps, agentic harnesses) no
  longer activate koopa by default, since activation can source `.envrc`
  files, secrets, and other side effects that are unsafe to run unattended.
  Opt in with `KOOPA_AUTO_ACTIVATE=1`. Interactive activation is unaffected.
- direnv activation is now bounded by `gtimeout` at both the shell-startup
  and `PROMPT_COMMAND` call sites, so a pathological `.envrc` (e.g. a hang)
  can no longer wedge the shell. Set `KOOPA_DIRENV_TIMEOUT=0` to disable.
- Fixed color-mode detection on headless Linux hosts: the XDG desktop
  portal query is now gated on an actual graphical session, avoiding a
  ~28s D-Bus activation stall on SSH/CI/SLURM hosts with no desktop, and a
  digit-matching bug that could misread the portal's `color-scheme` value.
  Color-mode activation also re-derives from the tmux server on every
  attach instead of trusting a stale inherited value.
- koopa can now run from a packaged (non-git) install prefix: `koopa_prefix()`
  honors `KOOPA_PREFIX` and falls back to `sys.prefix` for data files when no
  git-checkout tree is present, and `koopa_version()` falls back to installed
  package metadata. App install/reinstall/uninstall/update now refuse
  cleanly on such an install, since they self-update koopa via `git pull`.
  A draft (unsubmitted) conda-forge recipe exercises this groundwork.

Minor changes:

- `_koopa_check_multiple_users` and `_koopa_activate_today_bucket` now skip
  in non-interactive shells; both printed to stdout, which could corrupt
  `scp`/`rsync` output over SSH.
- Dotfiles configurer now raises on a broken (dangling) tree symlink instead
  of silently skipping that tree's install script.
- Python configurer platform resolution (`common` vs. a concrete OS id/family)
  is now consistent between `has_python_configurer` and
  `get_python_configurer`.
- AWS CLI errors now include stderr in the raised exception message.
- Download fallbacks (curl → `/usr/bin/curl` → urllib) now report why each
  attempt failed before retrying.
- `import_app_json()` raises `FileNotFoundError` instead of asserting on a
  missing `app.json`.
- Refactored the Xcode Command Line Tools check into three smaller helpers;
  behavior unchanged.
- Bumped bootstrap `uv` pin to 0.12.1.
- 66 app version bumps.

New apps:

- `actionlint` 1.7.12: static linter for GitHub Actions workflow files
  (conda-forge, non-default).
- `shfmt` 3.13.1: shell parser, formatter, and interpreter (conda-forge,
  non-default).
