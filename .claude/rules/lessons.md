...
| `koopa-r-release` | Acid Genomics R package release — cloud-native S3 + CloudFront, profile `acidgenomics`, CF distribution ID in `.env` (IDs/bucket in `cran.py`), `koopa app r publish/publish-from-github/reindex/archive`, sonoma-arm64 only active binaries, dep auto-install for binary builds, `_SKELETON_BINARY_PREFIXES` for big-sur-arm64 warning suppression, 16 packages still need Apache-2 relicense, pre-release gate via `AcidDevtools::check()`, impl in `cran.py` |
