...
| `koopa-shell-internals` | git recovery in `update_koopa()`, lazy-load vs eager init, activation fork budget + verify commands, cache-functions-before-reload ordering constraint |
...
| `koopa-color-mode` | SSH OSC 2031, env- vs file-driven timing, VS Code OSC 11 leak, targeted chezmoi apply, render-from-OS rule, never re-verify from agent session, abrupt-SSH-death terminal wedge (mouse+mode-2031 leak) + `koopa run reset-terminal` recovery |
