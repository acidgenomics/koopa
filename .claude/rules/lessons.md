...
| `koopa-r-release` | Acid Genomics R package release — r.acidgenomics.com drat repo (S3 `r-REDACTED_ACCOUNT_ID-us-east-1-an`, CF `REDACTED_CF_DIST_ID`, profile `acidgenomics`), `koopa app r publish/deploy/archive`, macOS arch caveat (one arch per machine), user-owned drat git commit, `AWS_CLOUDFRONT_DISTRIBUTION_ID_R` env var, impl in `cran.py` |
