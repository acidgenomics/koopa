...
| `koopa-color-mode` | SSH OSC 2031, env- vs file-driven timing, VS Code OSC 11 leak, targeted chezmoi apply, render-from-OS rule, never re-verify from agent session, abrupt-SSH-death terminal wedge (mouse+mode-2031 leak) + `koopa run reset-terminal` recovery, Linux gdbus `uint32` substring-collision stuck-light bug, POSIX `read` clobbering a successfully-read cache value, dead in-tmux re-derive in `activate-color-mode.sh` |
